...
class User
{
    ...
    public function save(): bool
    {
        if ($this->id) {
            ...
            $stmt = $this->database->prepare("
                ...
                VALUES (?, ?, ?, ?)
            ");
            $result = $stmt->execute([$this->name, $this->email, $this->password, $this->role ?? 'user']);
    ...
    public static function findAdmins(Database $database): array
    {
        $rows = $database->fetchAll("SELECT * FROM users WHERE role = 'admin' ORDER BY created_at DESC");
        $users = [];
    ...
    public static function findUsers(Database $database): array
    {
        $rows = $database->fetchAll("SELECT * FROM users WHERE role = 'user' ORDER BY created_at DESC");
        $users = [];
    ...
    public function delete(): bool
    {
        ...
        // Delete user's purchases and rentals (cascade delete)
        $stmt = $this->database->prepare("DELETE FROM purchases WHERE user_id = ?");
        $stmt->execute([$this->id]);

        $stmt = $this->database->prepare("DELETE FROM rentals WHERE user_id = ?");
        $stmt->execute([$this->id]);
